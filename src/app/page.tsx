...
export default async function HomePage() {
  ...
  if (supabase) {
    try {
      ...
      if (error) {
        console.error('Home page: failed to load pages from Supabase', error.message);
      } else if (data) {
        pages = data;
      }
    } catch (err) {
      console.error('Home page: Supabase request failed', err);
    }
  }
  ...
  for (const page of sorted) {
    ...
    const body = extractBodyContent(page.gjs_html);
  ...
  if (html && !isEmptyCmsHtml(html)) {
    const safeHtml = extractBodyContent(html);
    return (
      <div
        dangerouslySetInnerHTML={{
          __html: `<style>${css || ''}</style>${safeHtml}${contactSection}`,
        }}
      />
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto text-center">
            <p className="text-2xl md:text-4xl text-white/90 font-light leading-relaxed mb-10">
              &quot;我们制造世界上最好的家具。卓越品质，工厂直供价格，全球配送直达您的手中。&quot;
            </p>
            <a
              href="/about/"
            ...
            {[
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-white text-base md:text-xl font-bold uppercase tracking-wider drop-shadow-lg">
                    {cat.name}
                  </span>
                </div>
              </a>
